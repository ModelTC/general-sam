...
fn test_example_from_chars() {
    let sam_from_chars = GeneralSAM::<BTreeTransTable<char>>::construct_from_chars("abcbc".chars());
    // => GeneralSAM<char>
...
fn test_example_from_bytes() {
    let sam_from_bytes = GeneralSAM::<BTreeTransTable<u8>>::construct_from_bytes("abcbc");
    // => GeneralSAM<u8>
...
fn test_simple_bytes() {
    let sam = GeneralSAM::<BTreeTransTable<u8>>::construct_from_bytes("abcbc".as_bytes().iter());
...
fn test_simple_chars() {
    let sam = GeneralSAM::<BTreeTransTable<char>>::construct_from_chars("abcbc".chars());
    let state = sam.get_root_state();
    assert!(!state.is_accepting() && !state.is_nil() && state.is_root());
    let state = state.feed_chars("bc");
    assert!(state.is_accepting() && !state.is_nil() && !state.is_root());
...
fn test_chinese_bytes() {
    let sam = GeneralSAM::<BTreeTransTable<u8>>::construct_from_bytes("你好".as_bytes().iter());
    let state = sam.get_root_state();
    assert!(!state.is_accepting() && !state.is_nil() && state.is_root());
...
fn test_chinese_chars() {
    let sam = GeneralSAM::<BTreeTransTable<char>>::construct_from_chars("你好".chars());
    let state = sam.get_root_state();
    assert!(!state.is_accepting() && !state.is_nil() && state.is_root());
